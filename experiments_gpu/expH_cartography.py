...
The whole framework models each task's retained loss on a merge chart as a
convex quadratic. That assumption has never been looked at directly. Here we
map actual retained-loss surfaces densely on d=2 charts (three adapters), fit
the quadratic jet on the poised singles-and-pairs design, and measure how well
it reproduces the full surface, whether the fitted curvature is convex, and
where the quadratic breaks.

  python expH_cartography.py run --config expD_panel_m8.json --out results_expH
  python expH_cartography.py analyze --raw results_expH --out results_expH
"""
