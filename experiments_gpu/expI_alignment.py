...
The concurrent mergeability line finds gradient alignment the strongest signal
for merge success. This turns that signal into a subset feasibility screener on
the same m=8 answer-token panel and compares its AUROC against the atlas on
identical labels, so the interface table becomes a measured head-to-head.

For each task we take one adapter active, back-propagate the mean answer-token
loss to the base q/v-projection weights, and reduce each weight tensor's gradient
to a scalar by a fixed random projection (a per-module Johnson-Lindenstrauss
sketch). A subset is scored by the mean pairwise cosine alignment of its members'
gradient signatures; higher alignment predicts feasibility. Evaluated against the
answer-retention labels the atlas is scored on.

  python expI_alignment.py --config expD_panel_m8.json --rows results_expD/validation_rows.json --out results_expI
"""
