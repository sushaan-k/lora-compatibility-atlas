...
All capability evidence uses Mistral-7B because Lots-of-LoRAs is Mistral-only.
This trains one LoRA per task on a Qwen base (the family of the primary m=50
panel) from the same Natural Instructions tasks, so the answer-token screening
panel can be rebuilt on a second base and the one-base caveat removed.

Each adapter is a small SFT LoRA (r=16) on the task's train split, saved to its
own directory. Checkpointed per adapter so a preemption resumes.

  python train_second_base.py --config qwen_panel.json --out /root/atlas/qwen_adapters
"""
