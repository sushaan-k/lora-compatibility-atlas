...
The deployed intervention so far evaluates coefficient selection on a single
eight-way query. This experiment runs fifteen queries on the sixteen-adapter
Mistral chart, five each at orders four, five, and six, sampled with a fixed
seed. For each query the plan solves the zero-floor minimax over the query's
own support on the released m=16 answer-token jets (the same solver as the
eight-way plan) and records the atlas coefficient next to equal weights. The
run evaluates every member task of every query under both coefficients on
held-out instances [n_items : 2*n_items], disjoint from the fit split,
faithful to the eight-way protocol. The analysis compares the deployed worst
member answer loss and the mean member accuracy per query.

  plan    : CPU, from the released jets -> results_public/intervention_15q/plan.json
  run     : GPU -> deployed_rows.json    (resumable)
  analyze : CPU -> intervention_15q_summary.json

Run:  python3 expI_intervention15.py plan
      python3 expI_intervention15.py run --config m16_panel.json --plan plan.json --out DIR
      python3 expI_intervention15.py analyze --plan plan.json --rows DIR/deployed_rows.json --out DIR
"""
...
def cmd_plan(_):
    ...
    from foundations_bicore import load_jets, rho, w2c
    a, b, H = load_jets("m16")
    n = len(a)
