#!/usr/bin/env python3
"""Consolidation drift (B3): how far do jets move when an accepted merge is baked
into the base?

Section 4 flags the refit schedule after consolidation as an open problem. This
measures it: consolidate an accepted pair into the base (merge and unload), then
re-evaluate every other adapter's alone answer-token loss on the consolidated
base and compare to the original base. The drift is how far each surviving jet's
anchor moves, which sets whether the atlas must refit after a consolidation.

  python expJ_consolidation.py --config expD_panel_m8.json --pair task190+task1391 --out results_expJ
"""
