...
The capability channel has so far used linear merging. This refits the m=8
answer-token jets and revalidates every pair and triple under TIES and
DARE-TIES, so the operator breadth of the prompt-loss panels is reproduced on
labels that track accuracy. One operator per invocation.

  python expG_operators.py fit     --config C --operator ties --out RES
  python expG_operators.py validate --config C --operator ties --out RES

Analyze reuses expD_answer_panel.py analyze on the produced npz + rows.
"""
