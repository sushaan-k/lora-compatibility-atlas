...
The m=8 answer-token panel prescribes a denser design as the fix for the weak
triple-level accuracy discrimination, on the evidence of two fidelity points
(r=0.81 at 4.7x poisedness redundancy on the d=4 chart, r=0.52 at 1.0x on the
exactly-poised d=7 chart). This turns the anecdote into a curve.

Fix a held-out test set of genuinely off-design higher-order merges (4- and
5-way), evaluated once. Fit the eight answer-token jets on designs of increasing
size (singles+pairs, then with progressively more triples added), and for each
design predict the held-out merges and measure the prediction correlation and
the screening AUROC. Every model evaluation reports every task's answer-token CE
so the jets are refit cleanly at each level.

Run (GPU):
  python expE_density.py --config expD_panel_m8.json --out results_expE
Analyze (CPU, same or another machine):
  python expE_density.py analyze --raw results_expE --out results_expE
"""
...
from expC_answer_jets import (fit_jets, jet_value,  # noqa: E402
                              weights_to_coords)
