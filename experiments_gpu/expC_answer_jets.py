#!/usr/bin/env python3
"""Answer-token jets on the shared Mistral 5-adapter chart, validated against the
measured accuracies of Experiment A.

fit (GPU): for each design point on the shared 4-simplex over the five panel
adapters, build the linear merge and record each task's mean answer-token CE
(prompt masked) on its 40 held-out items. Fit one PSD-projected affine-quadratic
jet per task from these values. This is the atlas fitted on the capability channel.

analyze (CPU): evaluate the fitted jets at the coefficients of Experiment A's
measured merges, form predicted retention deficits, and score them against
accuracy-retention labels derived from A's measured accuracies
(retained iff acc >= base + tau_acc * (alone - base)). Reports AUROC of the
predicted deficit against the accuracy labels, per-task jet prediction quality,
and the shared-projector residual sweep for the answer-token Hessians.

Self-test (CPU, no GPU/data): python expC_answer_jets.py selftest
"""
