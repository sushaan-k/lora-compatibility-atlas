#!/usr/bin/env python3
"""Reliability figure (figs_png/reliability.pdf): empirical feasibility rate
against atlas-margin bucket median, per model family.

Adapted from the supplement's build_extra_diagnostics.py so that it writes
ONLY the figure.  Events use the linear operator; the margin is bucketed into
sixths of its distribution (quantile buckets; quartiles would be used below
n=60, which no panel here triggers).  Error bars are 95% Wilson intervals on
the per-bucket feasibility rate.

Rendered at final physical size (5.85in wide; the .tex includes it at
0.9\\linewidth on a 6.5in TMLR text block) with the shared 8pt boxed style.
"""
...
from fig_style import apply_style, BLUE
