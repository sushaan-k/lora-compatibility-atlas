...
Reconstructs the three geometry-free baselines on the balanced Qwen m=50
events from the released artifacts alone, under one documented convention
each, so the table regenerates from the release:

  single-gain floor : score a query by the weakest member's calibration gain,
                      gain(a) = base calibration loss - single-adapter
                      calibration loss, read from the matching (slice, method)
                      component summary in summary.json.
  learned baseline  : leave-one-slice-out logistic regression per operator
                      over [arity, min gain, mean gain] (no atlas features).
  clique count      : for triples, the number of feasible pair faces under
                      the face-closure pair run (qwen25_m50_facepairs);
                      label-aware, presumes a validated pair table.

AUROC is Mann-Whitney with 0.5 tie credit, positive class = feasible.

Run:  python3 scripts/qwen_cheap_baselines.py
Out:  results_public/qwen25_m50_balanced/cheap_baselines.json
"""
