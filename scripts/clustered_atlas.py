...
One library-wide projector over all fifty adapters needs rank 23 to reach a
strict max-task residual of 0.20 (Section 5). This script tests whether
partitioning the library into similarity-clusters recovers low certified rank
per cluster, so that a collection of small charts (an atlas in the literal
sense) certifies the compression the single global chart cannot.

For each adapter we describe its loss geometry by the top-r0 eigenspace of its
fitted Hessian H_t, cluster the adapters by subspace affinity, and for each
cluster fit one projector from the cluster mean Hessian. We report the
per-cluster sigma-adim against the library-wide r=23, an honest random-partition
baseline at matched sizes, the coverage tradeoff (which real query subsets stay
inside one cluster), and per-cluster interval certificates on the cluster faces.

All CPU, from results_public/shared_chart_decision_m50/qwen50_jets.npz.
"""
...
def main():
    ...
    (out / "clustered_atlas.json").write_text(json.dumps(result, indent=2))
    print("\nwrote", out / "clustered_atlas.json")
