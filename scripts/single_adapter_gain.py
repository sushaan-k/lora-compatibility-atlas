...
gain(t) = base_loss(t) - single_loss(t): positive means the adapter reduces the
held-out loss on its calibration probes. The feasibility normalisation divides by
this gain (clamped below at min_single_adapter_gain = 0.005), so gains at or below
the floor make the retention target degenerate to "stay near the base loss".
Reported in the Feasibility-rule and Limitations paragraphs.
"""
import json
import os
