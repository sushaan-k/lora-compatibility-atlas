#!/usr/bin/env python3
"""Operating-point analysis: does measured answer-token loss recover
accuracy-retaining merges at useful precision?

The paper's headline operating point (45% recall at 78% precision) is computed
on prompt-loss retention labels. Here we redo the same style of screening
analysis on CAPABILITY labels: a (merge, task) record is "retained" iff

    acc >= base_acc + tau_acc * (alone_acc - base_acc)

(the label convention of cmd_analyze in experiments_gpu/expC_answer_jets.py),
and we rank records by measured answer-token loss (ascending) vs measured
input-prompt loss (ascending) and report precision/recall at budget fractions.

Data: results_public/answer_token_panel/answer_token_panel.json
  - 5 rows merge=="base"  (per-task base-model accuracy/losses)
  - 5 rows merge=="alone" (per-task own-adapter accuracy/losses)
  - 45 rows: (merge, task) records. Structural note verified at runtime: every
    merge row's task is a member of the merge (the first-listed member), so the
    "tasks IN the merge" restriction keeps all 45 records and the
    all-(merge,task) pooled variant is identical. The 45 records cover 20
    distinct unordered merged models (pairs and triples) with incomplete
    member coverage.

No fabrication: every number is computed from the panel JSON.
"""
...
OUT_PATH = OUT_DIR / "answer_operating_point.json"
...
def main():
    ...
    out = {
        ...
        "label_convention": ("retained iff accuracy >= base_acc + tau_acc * "
                             "(alone_acc - base_acc), per expC_answer_jets.py "
                             "cmd_analyze"),
