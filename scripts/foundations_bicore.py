...
The two-sided score separates the protected task set T from the adapter set U with
a coefficient floor: rho(T;U) = min_{c in chart(U)} max_{t in T} q_t(c).  For each
jet library (alpha_t, b_t, H_t) on the (n-1)-simplex chart this measures, by
explicit core construction rather than subset enumeration:

  (1) exact atlas dimension r_exact = rank(sum_t H_t^2 + b_t b_t^T) and the
      Hessian-only rank, showing the linear terms add dimension;
  (2) task core (Helly): a greedy-minimal A* subset of T with rho(A*;U) = rho(T;U),
      of size <= r_exact + 1, whose members are the binding tasks;
  (3) adapter core (Caratheodory): a greedy-minimal V* subset of U with
      rho(T;V*) = rho(T;U), of size <= r_exact + 1;
  (4) discrete bicore saddle: beta(A*, V*) = rho(T;U), so the two cores together
      reconstruct the exact score;
  (5) task-core validity: dropping any A* member strictly relaxes the obstruction,
      and A* beats equal-size random task subsets;
  (6) rank-reduced cores and the 2*delta band: on rank-r jets the cores have size
      <= r + 1 and |rho_full - rho_reduced| <= 2*delta (Thm effdim_approx).

CPU only, from results_public jets.  Writes results_public/foundations/foundations.json.
"""
...
import expB_shared_chart_decision as expB  # noqa: E402,F401
from interval_certification import build_reduced_jets, eps_exact_certificate  # noqa: E402
