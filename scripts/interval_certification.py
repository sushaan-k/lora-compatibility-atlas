...
Upgrades the paper's "measured-exact" (r+1)-core agreement to a *certified*
per-library interval guarantee. For each substrate (qwen8, tinyllama) and each
r in {1,2,3}:

  1. Load the full jets (alpha_t, b_t, H_t) fit on the shared 7-simplex chart
     and build the rank-r reduced jets EXACTLY as expB_shared_chart_decision
     does: Pi_r = top-r eigenspace projector of the mean Hessian
     (expB.shared_projector), then (b, H) -> (Pi b, Pi H Pi)
     (expB.project_jet); alpha is unchanged, so dA_t = 0 identically.
  2. Certificate: on the chart {c : c_i >= 0, sum_i c_i <= 1} (coordinates are
     the first 7 of 8 simplex weights, expB.weights_to_coords), any quadratic
     difference delta(c) = dA + db.c + 0.5 c' dH c obeys
        |delta(c)| <= |dA| + max_i |db_i| + 0.5 max_ij |dH_ij|
     because 0 <= sum_i c_i <= 1 and sum_ij c_i c_j = (sum_i c_i)^2 <= 1.
     (The bound quoted for sum c_i = 1 holds a fortiori on this chart, where
     the last simplex weight absorbs the slack.) So
        eps_t := |dA_t| + max_i |db_t,i| + 0.5 max_ij |dH_t,ij|
     bounds |q_t - q^red_t| uniformly on the chart, and eps := max_t eps_t
     bounds |rho_S(full) - rho_S(reduced)| for EVERY subset S (min-max
     transfer: |min_c max_t f_t - min_c max_t g_t| <= max_t sup_c |f_t - g_t|).
  3. Enumerate all subsets S with |S| in {2,3,4,5} plus the full 8-set
     (28+56+70+56+1 = 211), score rho_full(S) and rho_reduced(S) with the SAME
     solver expB uses (expB.minimax_score: SLSQP epigraph over the FULL shared
     8-simplex, restarts=6, seed=0, min_weight=0), and form the (r+1)-core
     table of the REDUCED scores, i.e. max over sub-cores T of S with
     |T| <= r+1 of rho_reduced(T) (expB.core_score semantics; the core
     aggregation is a MAX, since rho_T <= rho_S for T subset of S).
  4. Verify empirically: (a) rho_reduced(S) equals its (r+1)-core value
     (Helly exactness for jets factoring through Pi_r), (b)
     max_S |rho_full - rho_reduced| <= eps, (c) tau sweep over
     [min score - 0.05, max score + 0.05] in 201 steps: decisions
     (rho <= tau) from the full jets vs. the (r+1)-core-reduced table agree
     100% whenever |rho_core - tau| > eps; report worst agreement inside the
     uncertified band |rho_core - tau| <= eps.
  5. Report score spread (max - min of rho_full over subsets) and eps/spread.

All minimax solves are memoized by frozenset(S) per jet family. The solver is
the SAME SLSQP epigraph program as expB.minimax_score (identical constraints,
bounds, start points, rng seed, and options), with ONE guard expB lacks:
expB takes min over restarts of res.fun without checking res.success or
feasibility, so a failed restart (e.g. SLSQP status 4, "inequality constraints
incompatible") can return an arbitrarily infeasible epigraph value and poison
the score. The guarded solver accepts a restart's res.fun only if the returned
point satisfies the epigraph and simplex constraints; an infeasible restart
instead contributes qmax at its clipped-and-renormalized weights (a feasible
point, hence a valid upper bound on the true minimum). Both the raw expB value
and the guarded value are recorded, and every subset where they differ is
listed as a solver artifact. This matters: on qwen8 the raw path returns
rho_reduced = -8.4e5 for one subset at r=3 (true value approx -50.1), and the
prior on-disk qwen8 r1 decide output contains a rho_full (-64.42 for one 4-way
query) that violates subset monotonicity against its own 2-core (-45.15), so
that artifact predates this script.
"""
...
import expB_shared_chart_decision as expB  # noqa: E402
...
def main():
    ...
    default_out = REPO / "results_public" / "interval_certification" / "interval_certification.json"
    ...
    results = {"description": "Per-library interval certificate for shared-chart "
               ...
               "solver": "expB_shared_chart_decision.minimax_score program "
