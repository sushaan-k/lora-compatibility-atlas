...
The published capability table (tab:capability in lora_compatibility_atlas_v2.tex)
quotes a subset-level pooled AUROC at the answer-retention threshold tau=0.70 for
each panel, together with a subset-level bootstrap CI. That CI treats the 84 (or
240) pair/triple subsets as independent draws. They are not: every subset is a
"+"-joined combination of a small pool of adapters (8 or 16), so pair and triple
queries that share an adapter share a task, an adapter, and correlated screening
errors. This script recomputes the exact published AUROC and then reports two
dependence-aware summaries that respect the adapter-sharing structure:

  1. Leave-one-adapter-out (LOAO): for each adapter a, drop every subset whose
     merge contains a and recompute the pooled AUROC on the survivors. The
     min-to-max range measures how much a single adapter drives the headline.

  2. Adapter-block bootstrap: resample the adapter pool with replacement (2000
     reps); keep a subset iff all of its members were drawn, weight it by the
     product of its members' resample multiplicities, and recompute a weighted
     AUROC. The 2.5/97.5 percentile interval is the block-bootstrap CI.

It also counts the infeasible subsets (negatives) at tau=0.70, because when that
count is tiny the AUROC rests on a handful of comparisons and is unstable.

The AUROC, the feasibility label, and the screening score are byte-for-byte the
same definitions used by experiments_gpu/expD_answer_panel.py (cmd_analyze):
...
Both are reconstructed from the per_member records already stored in each panel's
answer_panel_analysis.json, so no GPU or model weights are needed.

Run:  python scripts/capability_dependence_ci.py
Out:  results_public/capability_ci/capability_ci.json
"""
...
def auroc(labels, scores):
    """Exact copy of experiments_gpu/expC_answer_jets.py:auroc.
