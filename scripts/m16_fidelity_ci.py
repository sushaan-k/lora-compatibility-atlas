#!/usr/bin/env python3
"""Generator for results_public/capability_ci/m16_fidelity_ci.json.

Reconstructs the m=16 triple-fidelity uncertainty quoted in the capability
table caption from the released per-member records: the pooled Pearson of
predicted against measured answer loss on triple member events, its
leave-one-adapter-out range (drop every event whose merge contains adapter a),
and the adapter-block bootstrap interval (resample the 16-adapter pool with
replacement, keep events whose merge members were all drawn, weight by the
product of member multiplicities, recompute the weighted Pearson; 2000 reps,
seed 0). Asserts the recomputed point estimate matches the published value.

Run:  python3 scripts/m16_fidelity_ci.py
Out:  results_public/capability_ci/m16_fidelity_ci.json
"""
...
def main():
    ...
    (od / "m16_fidelity_ci.json").write_text(json.dumps(out, indent=1))
